...
class laserAvoid:
    ...
    def registerScan(self, scan_data):
        ...
        for i in range(len(ranges)):
            ...
            if 180-self.LaserAngle<angle<170:
                if ranges[i] < self.ResponseDist*1.0: self.Right_warning += 1
            if -170<angle<-180+self.LaserAngle:
                if ranges[i] < self.ResponseDist*1.0: self.Left_warning += 1
            if abs(angle) > 170:
                if ranges[i] <= self.ResponseDist*1.0: self.front_warning += 1
