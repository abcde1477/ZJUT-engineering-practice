<template>
  ...
  <div class="sign">
    <div>
      ...
      <div style="float:left;position:absolute;top:18vh;width:3.5vh;height:3.5vh;border:black 1px solid;background-color: #9d10ff"></div>
      <div style="float: left;margin-left: 2vw;margin-top: 1vh">:&nbsp;路径</div>
      <div style="float:left;position:absolute;top:22vh;width:3.5vh;height:3.5vh;border:black 1px solid;background-color: #f39c12"></div>
      <div style="float: left;margin-left: 2vw;margin-top: 1vh">:&nbsp;小车</div>
    </div>
  </div>
  <div class="info" >
...
async function fetchAndUpdateMap() {
  try {
    ...
    if (map[xCoordinate] !== null && map[xCoordinate][yCoordinate] !== null) {

      map[xCoordinate][yCoordinate] = 5;
      console.log(`坐标 (${xCoordinate}, ${yCoordinate}) 的值已更新为 5`);
    } else {
      console.error(`坐标 (${xCoordinate}, ${yCoordinate}) 超出地图范围或无效`);
    }
...
// 调用函数获取当前坐标并更新地图
fetchAndUpdateMap();
function updateobValue(map, x, y) {
  if (map[x] && map[x][y] !== undefined) {
    map[x][y] = 3;
    console.log(`坐标 (${x}, ${y}) 的值已更新为 5`);
  } else {
    console.error(`坐标 (${x}, ${y}) 超出地图范围或无效`);
  }
}

async function fetchobLocation() {
  try {
    ...
    const data = await response.json();
    console.log('获取的障碍物坐标数据:', data);

    // 将字符串类型的坐标转换为数字
    const xCoordinate = parseFloat(data.x);
    const yCoordinate = parseFloat(data.y);

    // 调用 updateMapValue 更新地图上的坐标
    updateobValue(map, xCoordinate, yCoordinate); // 将坐标的值更新为 5

    console.log(`当前障碍物坐标: (${xCoordinate}, ${yCoordinate})`);
