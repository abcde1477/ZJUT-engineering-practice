<template>
  <div class="pachong">
    ...
    <p v-if="weatherData">
      时间：{{ weatherData.time }},
      <br>
      实时气温：{{ weatherData.temperature }},
      <br>
      当前降水情况：{{ weatherData.precipitation }},
      <br>
      实时风速：{{ weatherData.wind_direction }},
      <br>
      实时风向：{{ weatherData.humidity }}
    </p>
    <p v-else>
  ...
  <div class="qrcode">
    <p1>二维码扫描</p1><br>
    <p2>您可以扫描二维码<br>扫描成功后即可识别货物</p2>
  </div>

  <div>
    <video style="position: absolute; top: 12vh; left: 2vw;" ref="video" width="660" height="520" autoplay></video>
    <canvas ref="canvas" hidden></canvas>
    <p id="result" style="font-size: 13px; position: absolute; top: 80vh; left: 52vw; width: 16vw; height: 14vh; background-color: white; color: black; text-align: center; padding: 10px; margin: 20px auto; word-wrap: break-word;">{{ qrCodeData }}</p>
    <button @click="toggleScan" style="position: absolute; top: 71vh; left: 65.4vw; width: 6vh; height: 6vh; background-color: #336699; color: white; border: none; cursor: pointer;">
      {{ scanning ? '停止扫描' : '启动扫描' }}
    </button>
  </div>
</template>
      ...
      if (this.video.readyState === this.video.HAVE_ENOUGH_DATA) {
        ...
        if (code) {
          ...
          try {
            ...
            this.qrCodeData = `QR码数据: ${JSON.stringify(qrData)}`; // 更新视图显示二维码数据
